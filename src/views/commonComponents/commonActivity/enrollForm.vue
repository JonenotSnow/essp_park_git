<template>
  <div class="enroll_form">
    ...
    <div class="enrollform_con">
      ...
      <div class="piaoj">
        <div class="pjdiv">
          <el-table current-row-key :data="tableData" style="width: 100%">
            <el-table-column align="center" prop="ticketType" label="票种" width="180">
            </el-table-column>
            <el-table-column prop="ticketPirce" align="center" label="价格" width="180">
              <template slot-scope="scope">
                {{scope.row.ticketPirce | changePrice}}
              </template>
            </el-table-column>
            <el-table-column prop="ticketNum" align="center" label="数量">
              <template slot-scope="scope">
                <div class="count esspclearfix">
                  <!-- 有待优化吧 -->
                  <div class="esspfl minus" v-if="scope.row.ticketNum > 0 && scope.row.ticketRemain > 0" @click="minus(scope.$index,scope.row)">
                    <i class="el-icon-minus"></i>
                  </div>
                  <div class="esspfl inputs myinput_no">
                    <input type="number" @change="changeNum(scope.$index,scope.row)" v-model="scope.row.ticketNum">
                  </div>
                  <div class="esspfl plus" @click="plus(scope.$index,scope.row)" v-if="scope.row.ticketRemain > scope.row.ticketNum ">
            ...
            <div v-for="(item,index) in nrollFormList" :key="index">
              <div v-for="(childItem,childIndex) in item" :key="childIndex" class="tablemain_item">
                <div class="tabletit esspclearfix">
                  <p class="text_align_l">票种:
                    <span>{{childItem.pName}}</span>
                  </p>
                ...
                <div class="tabmain" v-show="childItem.isShow">
                  ...
                  <div class="itemdiv" v-for="(formListItem, formListIndex) in childItem.formRqList" :key="formListIndex">
                    <label for="">
                      <em class="importanticon" v-if="formListItem.requir">*</em>{{formListItem.name + '：'}}</label>
                    <!--输入框-->
                    ...
                    <el-select v-if="formListItem.type=='radio'" v-model="formListItem.tittext" clearable placeholder="请选择">
                      <el-option v-for="item in formListItem.childrens" :key="item.name" :label="item.name" :value="item.name">
                      </el-option>
                    </el-select>
                    <!--复选框-->
                    <el-select multiple v-if="formListItem.type=='checkbox'" v-model="formListItem.tittext" clearable placeholder="请选择">
                      <el-option v-for="item in formListItem.childrens" :key="item.name" :label="item.name" :value="item.name">
                      </el-option>
                    </el-select>
                    <!--日期模版-->
                  ...
                  <div class="itemdiv" style="
                                        overflow: hidden;
                                        width: 160%;">
                    <el-button round class="tablebtn" size="mini" style="float: right;" @click="savePeopleInfo(childItem)">收起
                    </el-button>
                  </div>
                </div>
              </div>
          ...
          <div v-show="bookstyle == '2'" class="batchimport esspclearfix">
            ...
            <div class="batchbtn">
              <el-button size="small" type="primary" class="downloadbtn">
                <a class="down_a" :href="actData.enrolTemplateUrl">下载模板</a>
              </el-button>
              <el-upload class="uploadbtn upload-demo" action="#" :file-list="listFile" :before-upload="beforeup">
          ...
          <div class="tabmain" style="padding-bottom: 30px;">
            <div class="itemdiv" v-for="(item,index) in formRqList" :key="index">
              ...
              <el-select v-if="item.type=='radio'" v-model="item.tittext" clearable placeholder="请选择">
                <el-option v-for="item in item.childrens" :key="item.name" :label="item.name" :value="item.name">
                </el-option>
              </el-select>
              <!--复选框-->
              <el-select multiple v-if="item.type=='checkbox'" v-model="item.tittext" clearable placeholder="请选择">
                <el-option v-for="item in item.childrens" :key="item.name" :label="item.name" :value="item.name">
                </el-option>
              </el-select>
              <!--日期模版-->
      ...
      <el-dialog :visible.sync="enrollFormMaskCus" title="提示" class="enroll_success enroll_succ">
        <p class="icon_p icon_p1">
          <i class="el-icon-warning"></i>是否确认取消活动报名</p>
      ...
      <el-dialog :visible.sync="enrollFormMask" class="enroll_success">
        <div class="icon_div">
...
export default {
    ...
    methods: {
                ...
                _that.tableData.forEach((item, index) => {
                    ...
                    for (let j = 0; j < use; j++) {
                        if (item.ticketType == _that.ticketUse[j].ticketType) {
                            let ticketRemain =
                                item.ticketNum - _that.ticketUse[j].ticketUse;
        ...
        changeNum(index, item) {
            ...
            if (item.ticketNum > item.ticketRemain) {
                item.ticketNum = item.ticketRemain;
                this.$message.error(
                    "票数量不能大于" + item.ticketRemain + "张"
                );
            }
            ...
            if (diff > 0) {
                ...
                this.nrollFormList[index].splice(
                    this.nrollFormList[index].length - Math.abs(diff),
                    Math.abs(diff)
                );
            }
        },
        changeFormList(index, type) {
            ...
            if (type === 1) {
                this.nrollFormList[index].splice(
                    this.nrollFormList[index].length - 1,
                    1
                );
            } else {
                this.nrollFormList[index].push(info);
