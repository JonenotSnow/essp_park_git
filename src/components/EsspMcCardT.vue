<template>
  <div class="essp-card-cont-inner esspclearfix">
    <div class="essp-card" v-for="(item, mcCardIndex) in mcCardList" v-if="mcCardList.length>0" :key="mcCardIndex">
      ...
      <div class="cont-detail">
        ...
        <div class="cont-detail-l">
          <p class="tag_btn_p">
            <essp-park-tag v-for="(item, eptIndex) in activityLabelList[mcCardIndex]" :value="item" v-if="eptIndex<3" :key="eptIndex" />
          </p>
          <p>
        ...
        <div class="cont-detail-r 123" v-if="LoginUserRol == '11'">
          <button type="primary" v-if="item.btn.length>0" class="essp-btn btn-cancle" v-for="(itemChild,index) in item.btn" :key="index">{{itemChild.name}}
          </button>
        </div>
        <div class="cont-detail-r  456" v-else>
          <button type="primary" class="essp-btn" :class="itemChild.clicked?'btn-enroll':'btn-cancle'" v-for="(itemChild,index) in item.btn" :key="index" @click="enrollToPage(itemChild,item,mcCardIndex)">{{itemChild.name}}
          </button>
        </div>
    ...
    <div v-if="mcCardList.length==0" style="text-align: center">{{isLodingTxt}}</div>
